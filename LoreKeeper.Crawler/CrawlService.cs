...
public sealed class CrawlService : BackgroundService
{
    ...
    public CrawlService(IHttpClientFactory httpFactory, CrawlerConfig config, IDbBootstrapper bootstrapper, IPagesRepository repo)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        foreach (var baseUrl in _config.Wikis)
        {
            ...
            do
            {
                var listUrl = $"{baseUrl}/api.php?action=query&format=json&formatversion=2&list=categorymembers&cmtitle={Uri.EscapeDataString(_config.SeedCategory)}&cmlimit=100" +
                    ...
                    query.TryGetProperty("categorymembers", out var members))
                {
                    foreach (var m in members.EnumerateArray())
                    {
                        ...
                        var parseUrl = $"{baseUrl}/api.php?action=parse&pageid={pageId}&prop=text|sections|links|categories&format=json&formatversion=2";
                        ...
                        {
                            if (parse.TryGetProperty("text", out var text) && text.TryGetProperty("*", out var htmlEl))
